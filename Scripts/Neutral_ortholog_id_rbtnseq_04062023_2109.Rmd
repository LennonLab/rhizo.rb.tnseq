...
#data
```{r}
matches = read.delim("~/Github/rhizo.rb.tnseq/Scripts/rbh_essentiality_supplies/out_smeliplant.txt",header=FALSE)
colnames(matches) <- c("wheatleyID", "rbtnseqID")
non_estl = read.csv("~/Github/rhizo.rb.tnseq/Scripts/rbh_essentiality_supplies/Rlv_nonessential gene list.csv")
non_estl$estl <- rep('N',nrow(non_estl))
tnseq_genes_old = read.csv("~/Github/rhizo.rb.tnseq/Scripts/rbh_essentiality_supplies/old_annotations_smeliplant.csv")
tnseq_genes_prot = read.csv("~/Github/rhizo.rb.tnseq/Scripts/rbh_essentiality_supplies/proteins_smeli.csv")

sog = read.csv("~/Github/rhizo.rb.tnseq/Scripts/rbh_essentiality_supplies/aaseq2_smeliplant.csv") #sog = single orthologous genes
```
...
write.csv(sog,'~/Github/rhizo.rb.tnseq/Scripts/rbh_essentiality_supplies/smeliplant_neutral.csv')
```

#Add the annotation data from eggnog mapper
```{r}
cogs = read.csv("~/Github/rhizo.rb.tnseq/Scripts/rbh_essentiality_supplies/emapper_analysis_smeliplant.csv")
...
write.csv(cogs,'~/Github/rhizo.rb.tnseq/Scripts/rbh_essentiality_supplies/smeliplant_neutral_cog.csv')
```
