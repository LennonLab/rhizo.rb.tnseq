...
#The following data is the fitness scores generated from the FEBA pipeline
...
#goodfitdat <- fitdat[, c("X21", "X22", "X24", "X26", "X27", "X28", "X29", "X30", "X31", "X32", "X35", "X37", "X40", "X41", "X42", "X43", "X44", "X45")] #more than 2 million reads
goodfitdat = fitdat

#COUNT DATA for analyses that can't use log ratio data for whatever reason (negative numbers in data, log scale, etc)
...
#removes description from column names, leaving only locus id
...
row.names(goodqual_metadata) <- c("X21", "X22", "X24", "X26", "X27", "X28", "X29", "X30", "X31", "X32", "X35", "X37", "X40", "X41", "X42", "X43", "X44", "X45") #adds numerical IDs to metadata df
...
fitmetadata <- fitmetadata[, -c(1)] #clean up empty column
...
nitrogengenes$V1[nitronames] <- gsub("\\s+$", "", nitrogengenes$V1[nitronames]) #same name-cleaning process as above; I wrote this before I knew about trimws()
...
KEGGNmetabolism <- Nmetabolismfit #KEGG df clones to preserve metadata for later

nodESfit <- nodESfit[, -c(2:18)]
nodNEfit <- nodNEfit[, -c(2:18)]
completefit <- completefit[, -c(2:18)]
Nmetabolismfit <- Nmetabolismfit[, -c(2:18)] #removes metadata from subsetted groups to get only the numerical data in the frame itself
...
dissim_Nmetabolism <- vegdist(Nmetabolismfit, method = "euclid", diag = FALSE) #resemblance matrices
...
rownames(combined_nodES) <- rownames(ranked_nodES_nodamb) #match row names
...
scatterplot_list <- list() #store results

for (test_name in all_spearman) {
  spearman_test <- get(test_name) #get data from listed name
...
#ES
finalfitrank_nodES <- as.data.frame(rbind(as.numeric(ks_ranked_nodES_bactamb), as.numeric(ks_ranked_nodES_bactfert), as.numeric(ks_ranked_nodES_nodamb), as.numeric(ks_ranked_nodES_nodfert), as.numeric(ks_ranked_nodES_t0))) #combine rankings to be plotted together
colnames(finalfitrank_nodES) <- 1:832
finalfitrank_nodES$treatment <- c("bact_amb", "bact_fert", "nod_amb", "nod_fert", "t0")

finalfitrank_nodES <- pivot_longer(finalfitrank_nodES, cols = colnames(finalfitrank_nodES)[1:832]) #flips df for plotting purposes
...
p_values <- numeric(length(ks_test_objects)) #make a landing zone for the pvals and IDs that's as long as the input df
...
names(p_values) <- gsub("^ks_", "", ks_test_objects) #name them according to the test they came from

adjusted_pvals_ks <- p.adjust(p_values, method = "fdr") #run the FDR and store results
...
rownames(ks_results_table) <- ks_objects #set row names to corresponding K-S test
...
row_names_Nmetab <- c("bact_amb/nod_amb", "bact_amb/bact_N", "bact_N/nod_N", "nod_amb/nod_N") #row names
...
rownames(fitrank_Nmetab) <- row_names_Nmetab #pull ks test statistics and set row names to name the comparison more clearly

```

```{r Chunk 8: Covariance PCA + plots}

cor_nodES <- cor(covar_nodES)
cor_nodNE <- cor(covar_nodNE)
cor_complete <- cor(covar_complete)
cor_Nmetabolism <- cor(covar_Nmetab) #correlation matrices for KMO and bartlett tests
...
``` {r Chunk 9: COG analyses}


#SUBSET DATA BY COG FOR DOWNSTREAM ANALYSES


COGcomplete_conting <- COGcomplete #made a copy of the df so i could work with it without destroying it
COGnumeric_cols <- which(sapply(COGcomplete_conting, is.numeric)) #creates separate df extracting numeric data from character metadata by applying is.numeric to each element and selecting which() objects pass the test (ie evaluate to TRUE)

for (col in COGnumeric_cols) {
  COGcomplete_conting[[col]] <- ifelse(COGcomplete_conting[[col]] <= -1, -1,
                                       ifelse(COGcomplete_conting[[col]] >= 1, 1, 0))
} #preps data for contingency tables by replacing low fitness values w -1, high ones w 1, neutral w 0; modifies COGcomplete_conting
...
COGlist <- lapply(unique(COGmatrices$COG), function(currCOG) { #obtains list of COGs and runs currCOG for each
  COG_X <- COGmatrices %>%
    filter(COG == currCOG) %>%
    as.data.frame()
  return(COG_X) #makes df for each unique COG consisting of only all the genes of that COG
})

for (i in seq_along(COGlist)) {
  currCOG <- unique(COGlist[[i]]$COG)
  assign(paste0("COG_", currCOG), COGlist[[i]])
} #assign each COG_X df to a variable with the corresponding name in GlobalEnv


#MAKE CONTINGENCY TABLES


COGnames <- ls(pattern = "^COG_")  #only the COG_X dfs are named starting with "COG_"; other objects starting with "COG" have no _
for (df_name in COGnames) {
  df <- get(df_name) 
  df <- df[, c(5, 19:31)] #isolate necessary metadata and remove t0
  assign(df_name, df, envir = globalenv())
}

COGnames <- COGnames[! COGnames %in% c('COG_B', 'COG_W')]  #get rid of tables with only 1 gene counted


#make stuff for fisher tests


for (df_name in COGnames) {
  df <- get(df_name)
    negatives <- rowSums(df[, 2:14] == -1)
  zeros <- rowSums(df[, 2:14] == 0)
  positives <- rowSums(df[, 2:14] == 1) #total up the frequency of 1s, 0s, and -1s in the fitness values of each df
  
  condensed_df <- df %>% 
    select(essentiality) %>% #divide by essentiality
    mutate(negative = negatives, neutral = zeros, positive = positives) #replace fitness data with tabulated counts
    assign(df_name, condensed_df, envir = globalenv()) #updates dfs in global env
}

COGtables <- vector("character", length = length(COGnames) * 2)  #empty vector to store the table names
for (i in seq_along(COGnames)) {
  df_name <- COGnames[i]
  df <- get(df_name)
  df_name <- substr(df_name, 1, 5)  #extract "COG_X" name to be assigned to output dfs
  
  positive_essential <- sum(df$positive[df$essentiality == "essential"])
  positive_N <- sum(df$positive[df$essentiality == "N"]) #total up positive column for each category
  neutral_essential <- sum(df$neutral[df$essentiality == "essential"])
  neutral_N <- sum(df$neutral[df$essentiality == "N"]) #total up neutral for each category
  
  df_positive <- data.frame(
    positive = c(positive_essential, positive_N) / 13,
    neutral = c(neutral_essential, neutral_N) / 13
  )   #create the data frame for COG_X_positive and set row names

  
  df_positive <- round(df_positive)
  
  rownames(df_positive) <- c("essential", "N")   #round the values to the nearest whole number
  
  df_positive_name <- paste0(df_name, "_positive")
  assign(df_positive_name, df_positive, envir = globalenv())   #assign finished COG_X_positive df to global environment
  
  COGtables[(i * 2) - 1] <- df_positive_name   #add the name of the data frame to the COGtables list
  
  negative_essential <- sum(df$negative[df$essentiality == "essential"])
  negative_N <- sum(df$negative[df$essentiality == "N"]) #total up negative column for each category
  neutral_essential <- sum(df$neutral[df$essentiality == "essential"])
  neutral_N <- sum(df$neutral[df$essentiality == "N"]) #total up neutral column for each category (redundant for organization)
    
  df_negative <- data.frame(
    negative = c(negative_essential, negative_N) / 13,  #divide by 13 because 13 reps
    neutral = c(neutral_essential, neutral_N) / 13
  )   #create the data frame for COG_X_negative and set row names
  
  df_negative <- round(df_negative)
  
  rownames(df_negative) <- c("essential", "N")   #round the values to the nearest whole number
  
  df_negative_name <- paste0(df_name, "_negative")
  assign(df_negative_name, df_negative, envir = globalenv())   #assign finished COG_X_negative df to global environment
  
  COGtables[i * 2] <- df_negative_name   #add the name of the data frame to the COGtables list
} #tabulate and make contingency tables, naming each as an individual df in globalenv


#FISHER TESTS


for (table_name in COGtables) {
   df <- get(table_name)
   result_name <- paste0("fisher_", table_name)
   assign(result_name, fisher.test(df, simulate.p.value = FALSE))
}

fisher_tests <- ls(pattern = "fisher_COG", envir = .GlobalEnv)

fisher_pvals <- numeric(length(fisher_tests))

for (j in seq_along(fisher_tests)) {
  fisher_pvals[j] <- get(fisher_tests[j])$p.value
} 

names(fisher_pvals) <- gsub("^fisher_", "", fisher_tests)

adjusted_fisher_pvals <- p.adjust(fisher_pvals, method = "fdr")

```

``` {r Chunk 10: fgsea and geseca for COGs}
...
pathways_ES <- gmtPathways("COGsets_ES.gmt") #I added this renamed file to my R working directory
...
tlike_data <- tlike_data[, -c(2:3)] #removes redundant col and description
tlike_data <- tlike_data[, -c(18:22)] #removes t0
...
tlike_col_ranges <- list(
  ...
  bactN = c(11:13),
  bactamb = c(14:15)
) #defines ranges of columns containing data for each treatment/compartment combination to be averaged together

tlike_data_meaned <- data.frame(ID = tlike_data$locusId) #creates df to be populated by meaned t-like data

for (i in 1:nrow(tlike_data)) {
  new_values <- numeric(length(tlike_col_ranges))  #goes through each row of tlike_data and creates new df of the proper length with the same rows as the parent
...
print(tlike_data_meaned) #make sure it looks right
row.names(tlike_data_meaned) <- tlike_data_meaned[, 1]
tlike_data_meaned <- tlike_data_meaned[, -c(1)] #clean up extra row containing row names
...
bactambranks <- read.table("C:/Users/User/Desktop/bactamb_ranked.rnk", header=TRUE, colClasses = c("character", "numeric")) #reads .rnk files back in as ranks
...
geseca_data <- geseca_data[, -c(1)] #the normalize.quantiles function erases row names so some shenanigans are needed
...
newtags <- newtags[, -c(6)] #remove extraneous column used to generate csv file for reannotation

newtags$KEGG_ko <- NA #ditto
...
newtag_KEGG <- read.csv("C:/Users/User/Desktop/new_locustags_keggko.csv") #df containing tags matched by blastKOALA to new locus tags, in case we can get more KEGG information that way
...
KEGGfitmetadata <- KEGGfitmetadata %>%
  separate_rows(KEGG_ko, sep = "/") #splits and duplicates rows with kos like this: id/id. These are separate genes in a two-gene operon which are annotated as one for some


#PREP DATA FOR CONTINGENCY TABLES


KEGGnumeric_cols <- which(sapply(KEGGfitmetadata, is.numeric)) #same process as COG analysis; picks out numeric data to be worked with
...
KEGGfitmetadata$KEGG_Pathway <- trimws(KEGGfitmetadata$KEGG_Pathway) #clean up pathway row before shuffling stuff around
...
KEGGfit_env <- new.env() #new env to store all the pathway dfs
KEGGfit_list <- split(KEGGfitmetadata, KEGGfitmetadata$KEGG_Pathway) #split KEGGfitmetadata into list of dfs based on pathway ids
...
rm("KEGG_-", envir = KEGGfit_env) #delete subset containing unannotated genes
...
KEGGfit_env <- lapply(KEGGfit_env, function(df) {
  ...
  return(NULL)
}) #deletes gene sets with fewer than 6 genes represented, as these probably make uselessly tiny tables? Will check
...
KEGGfit_env <- KEGGfit_env[!sapply(KEGGfit_env, is.null)] #clean up all the nulled stuff

#condense categorical data into row totals

condensed_dfs <- list() #make empty list for condensed_dfs to be put into KEGGfit env at the end
df_names <- ls(KEGGfit_env) #have to do things this way since KEGGfit_env was made with new.env()
...
ko_tables <- vector("character", length = length(ls(KEGGfit_env)) * 2) #empty vector to store table names
df_index <- 1 #keep track of df index in loop (weird system suggested by chatGPT after several attempts to manually adapt the COGtables loop for this purpose)
...
rm(list = kegg_source_objects, envir = KEGGfit_env) #delete source dfs, leaving only the tabulated contingency tables 
...
fisher_KEGG <- ls(pattern = "^fisher_ko", envir = KEGGfit_env) #retrieve KEGG fisher tests
...
adjusted_fisher_KEGG_pvals <- p.adjust(fisher_KEGG_pvals, method = "fdr") #trim names and fdr adjust p vals
...
KEGGfit_pathwaysets <- KEGGfit_pathwaysets[sapply(KEGGfit_pathwaysets, function(x) nrow(x[[1]]) >= 4 && nrow(x[[2]]) >= 4)] #removes very small pathways that can create spurious hits
...
KEGGfit_pathwaysets <- lapply(KEGGfit_pathwaysets, function(tibble_list) {
  ...
  tibble_list
}) #removes t0 columns from analysis
...
genesetsKEGG_NE <- unlist(map(KEGGfit_pathways_NE, get_genesets), recursive = FALSE) #stores ES and NE sets as lists of character vectors and removes the redundant level of organization
writeGmtPathways(genesetsKEGG_ES, tempfile("KEGGsets_ES", fileext = ".gmt"))
writeGmtPathways(genesetsKEGG_NE, tempfile("KEGGsets_NE", fileext = ".gmt")) #make pathways file for fgsea

pathways_keggES <- gmtPathways("KEGGsets_ES.gmt")
pathways_keggNE <- gmtPathways("KEGGsets_NE.gmt") #it takes a lot of steps to make the .gmt file for some reason
...
#   df <- get(df_name)
#   df <- df[!grepl("set-|set_-", df[, 1]), ] #remove unannotated sets from results
#   assign(df_name, df, envir = .GlobalEnv)
# } #commenting this out because the unannotated genes might be interesting? idk
...
df_list <- list() #empty list to store collated fgsea dfs for easy plotting
...
fgsea_collated <- bind_rows(df_list) #combine

fgsea_collated <- fgsea_collated %>%
  filter(!grepl("set-|set_-", pathway)) #remove unannotated sets from data

fgsea_collated_ES <- fgsea_collated %>%
  filter(grepl("_ES$", group))
fgsea_collated_NE <- fgsea_collated %>%
  filter(grepl("_NE$", group)) #split into ES and NE for plotting purposes

fgsea_collated_ES$group <- sub("_ES$", "", fgsea_collated_ES$group)
fgsea_collated_NE$group <- sub("_NE$", "", fgsea_collated_NE$group) #remove redundant tags
fgsea_collated_ES$group <- sub("fgsea_", "", fgsea_collated_ES$group)
fgsea_collated_NE$group <- sub("fgsea_", "", fgsea_collated_NE$group) #trim down to just the treatment/compartment combination

fgsea_plotting <- new.env() #create paired dfs for easy plotting of comparisons
...
``` {r Chunk 13: Gene-level ANOVAs and leading-edge}
...
all_gene_ids <- unique(unlist(as.list(leadingEdge_extracted))) #get only unique gene ids from the env (may be redundant)
...
row.names(leadingEdge_allgenes) <- NULL #clean up weird row names


#K-S rank-order vs gene order cor tests

split_dfs <- split(rankfitdata, rankfitdata$V1) #splits rank fitness data into treatment groups
names(split_dfs) <- paste0(names(split_dfs), "_genomeorder")
split_dfs <- lapply(split_dfs, function(df) df[, -which(names(df) == "V1")]) #deletes extra col
...
for (i in seq_along(list_of_vectors)) {
  ...
  rownames(df) <- colnames(list_of_vectors[[i]]) #converts double to df, preserving row/col names
...
replicate_names <- gsub("_genome", "_rank", replicate_names, fixed = TRUE) #modify names to reflect re-ordering
names(rankorder_replicates) <- replicate_names #reassign

rankorder_replicates <- lapply(rankorder_replicates, t) #flips them to same orientation as genome order objects
...
rankshift_test <- cor.test(genomeorder_replicates[["bact_ambient_genomeorder_037"]], rankorder_replicates[["bact_ambient_rankorder_037"]], method = "pearson")

rankshift_test2 <- cor.test(genomeorder_replicates[["bact_ambient_genomeorder_037"]], rankorder_replicates[["bact_ambient_rankorder_040"]], method = "pearson")


rankshift_test3 <- cor.test(genomeorder_replicates[["bact_ambient_genomeorder_037"]], genomeorder_replicates[["bact_ambient_genomeorder_040"]])

#NOTE: more code needed to make big cor matrices and run omnibus cor tests assuming we get this weirdness ironed out
...
for (df_name in all_fgsea_cog) {
  df <- get(df_name)
    ...
    assign(df_name, df, envir = .GlobalEnv)
}#gets rid of unannotated result sets
