...
kruskal.test(Shannon ~ group, data = alphadiversitylib)
kruskal.test(SW_Evenness ~ group, data = alphadiversitylib)
kruskal.test(percentFound ~ group, data = alphadiversitylib)
kruskal.test(Richness ~ group, data = alphadiversitylib)
...
dissim_Nmetabolism <- vegdist(Nmetabolismfit, method = "euclid", diag = FALSE) #resemblance matrices; still experimenting with different dissimilarity indices. Fitness data is log ratio data so it's kind of weird
...
permanova_pvals <- c(nodES_permanova$`Pr(>F)`, nodNE_permanova$`Pr(>F)`)

permanova_pvals <- na.omit(permanova_pvals)

adjusted_permanova_pvals <- p.adjust(permanova_pvals, method = "fdr")

#need to think about the architecture of this a little more--ideally, we would stratify by pot, but this isn't feasible with the bacteroid and nodule data split across the pots like this (nodule data from pots 1, 2, 4, 6, 7, 8, 9, 10; bacteroid data from pots 2, 3, 5, 7, 9, 10)
```
...
# NOTE: decided to go with fitness ranking analysis exclusively; leaving this code in for documentation purposes and in case we need to give it a second look. Leaving RACsubsetter uncommented because it is reused for the fitrank
...
covar_permanova_pvals <- c(nodES_covar_permanova$`Pr(>F)`, nodNE_covar_permanova$`Pr(>F)`, Nmetab_covar_permanova$`Pr(>F)`)
...
``` {r Chunk 12: fgsea and geseca for COGs}
...
for (fgsea_name in all_fgsea_cog) {
  ...
  print(plotGseaTable(pathways_file[unique(topPathways)], ranks_file, fgsea_obj, gseaParam = 0.5))
...
``` {r Chunk 15: Gene-level contingency table tests}
...
# Deprecated code for nested ANOVA by gene
#
...
# 
# ANOVAfitness <- data.frame(Ntreatment = c(rep("fertilized",3), rep("ambient",5),rep("fertilized", 3), rep("ambient", 2)), compartment = c(rep("nodule",8), rep("bacteroid",5)), strata = CMHstrata, ANOVAfitness)
# 
# anovafit_results <- list()
# for (i in 3:ncol(ANOVAfitness)) { #go through each of the gene fitness data cols
#   col_name <- colnames(ANOVAfitness)[i]
#   formula <- as.formula(paste(col_name, "~ Ntreatment/compartment + strata")) #write formula for current anova
#   anova_result <- aov(formula, data = ANOVAfitness) 
#   anovafit_results[[col_name]] <- anova_result #run analysis and store results
# }
