...
#next step: pull locus ids from each COG_X df into vectors to be fed into a pathways.gmt file for fgsea analysis

genesetsCOG <- lapply(seq_along(COGlist), function(i) {
  df <- COGlist[[i]]
...
genesetsCOG <- unlist(genesetsCOG, recursive = FALSE) #final format

#then, pipe the list into writeGmtPathways(); declare pathways <- Gmtpathways(written)

writeGmtPathways(genesetsCOG, tempfile("COGsets", fileext = ".gmt")) #creates file in temp dir which must be manually retrieved, also includes like a hexadecimal id (or maybe it's just random numbers and letters?) which I deleted
pathways <- gmtPathways("COGsets.gmt") #I added this renamed file to my R working directory
...
# write.table(tlike_bactamb_ranked, file = "C:/Users/User/Desktop/bactamb_ranked.rnk", sep = "\t", quote = FALSE, row.names = TRUE) #creates tab-delimited .rnk files for gene set enrichment analysis
...
#then, run geseca() and fgsea()

fgsea_nodN <- fgsea(pathways, nodN_ranks, minSize = 1, maxSize = 1100)
fgsea_nodamb <- fgsea(pathways, nodamb_ranks, minSize = 1, maxSize = 1100)
fgsea_bactN <- fgsea(pathways, bactN_ranks, minSize = 1, maxSize = 1100)
fgsea_bactamb <- fgsea(pathways, bactamb_ranks, minSize = 1, maxSize = 1100)

geseca_data <- tlike_data
rownames(geseca_data) <- geseca_data$locusId
geseca_data <- geseca_data[, -c(1)]

geseca_rbtnseq <- geseca(pathways, geseca_data, minSize = 1, maxSize = 1100)
