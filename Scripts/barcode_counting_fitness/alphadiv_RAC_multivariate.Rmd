...
metadata = as.data.frame(matrix(ncol=2,nrow=19))
metadata[,1] <-c(rep("nod",9),rep("bact",5),rep("start",5))
metadata[,2] <-c(rep("fertilized",4),rep("ambient",5),rep("fertilized",3),rep("ambient",2),rep("time0",5))
...
row.names(labeled_metadata) <- c("X21", "X22", "X23", "X24", "X26", "X27", "X28", "X29", "X30", "X31", "X32", "X35", "X37", "X40", "X41", "X42", "X43", "X44", "X45") #adds numerical IDs to metadata df

goodqual_metadata <- subset(labeled_metadata, row.names(labeled_metadata) %in% c("X21", "X22", "X24", "X26", "X27", "X28", "X29", "X30", "X31", "X32", "X35", "X37", "X40", "X41", "X42", "X43", "X44", "X45"))
potlabels <- c("1", "2", "4", "6", "7", "8", "9", "10", "1", "2", "5", "7", "10", "t0", "t0", "t0", "t0", "t0") 
...
alphadiversitylib$group <- c("nodN", "nodN", "nodN", "nodamb", "nodamb", "nodamb", "nodamb", "nodamb", "bactN", "bactN", "bactN", "bactamb", "bactamb", "t0", "t0", "t0", "t0", "t0")
...
alphadiv_anova_data <- alphadiversitylib %>%
  ...
  ) #grouped and meaned for anova, and also for reporting mean and SE

alpha_anova_shannon <- aov(Shannon ~ group, data = alphadiversitylib)
alpha_anova_SWevenness <- aov(SW_Evenness ~ group, data = alphadiversitylib)
alpha_anova_percentfound <- aov(percentFound ~ group, data = alphadiversitylib)
alpha_anova_richness <- aov(Richness ~ group, data = alphadiversitylib)

alphadiv_onlyresponse <- head(alphadiversitylib, -5)

alpha_anova_shannon_exp <- aov(Shannon ~ group, data = alphadiv_onlyresponse)
alpha_anova_SWevenness_exp <- aov(SW_Evenness ~ group, data = alphadiv_onlyresponse)
alpha_anova_percentfound_exp <- aov(percentFound ~ group, data = alphadiv_onlyresponse)
alpha_anova_richness_exp <- aov(Richness ~ group, data = alphadiv_onlyresponse)
...
row.names(RACmetadata) <- c("021", "022", "023", "024", "026", "027", "028", "029", "030", "031", "032", "035", "037", "040", "041", "042", "043", "044", "045")
RACmetadata <- subset(RACmetadata, row.names(RACmetadata) %in% c("021", "022", "024", "026", "027", "028", "029", "030", "031", "032", "035", "037", "040", "041", "042", "043", "044", "045"))
...
finalfitrank_nodES <- pivot_longer(finalfitrank_nodES, cols = colnames(finalfitrank_nodES)[1:2756]) #flips df for plotting purposes
...
ks_objects <- ls(pattern = "^ks_", envir = .GlobalEnv)
...
covar_permanova_pvals <- c(nodES_covar_permanova$`Pr(>F)`, nodNE_covar_permanova$`Pr(>F)`)
...
COGtables <- COGtables[! COGtables %in% c('COG_B_NE_table', 'COG_W_ES_table')] #get rid of tables with only 1 gene counted

for (table_name in COGtables) {
   df <- get(table_name)
   char_index <- 5
   result_name <- paste0("fisher_cog", substr(table_name, char_index, char_index), "_", substr(table_name, 7, 8))
   assign(result_name, fisher.test(df, simulate.p.value = TRUE))
}
...
#PULL LOCUS IDS FROM EACH COG_X DF INTO VECTORS TO MAKE PATHWAYS.GMT FOR FGSEA
...
tlike_data <- read.delim("C:/Users/User/Desktop/thesis_stuff/R_working/fitness_data/html/SmeliPlant/fit_t.tab")
...
#preps data for contingency tables by replacing low fitness values w -1, high ones w 1, neutral w 0, and deletes NAs created by blastkoala annotation including genes which are fatal upon insertional inactivation
...
#next steps for KEGG stuff: make list of dfs for ES/NE of each pathway set -> unlist into over 400 individual dfs (bad idea??) -> relist names in a list -> remove t0 and other unnecessary columns from each df in list -> define col ranges corresponding to each t/c combination and sum together, make empty contingency tables and labels, name according to represented pathway -> make list of all empty table dfs -> sum each treatment together for each pathway set and fill in corresponding table with summed values 

#Contingency tables and Fisher tests; need to figure out how to make tables by pathway id
...
turnover_fit_bacteroid <- fit_turnoverdat[9:14 ,]
