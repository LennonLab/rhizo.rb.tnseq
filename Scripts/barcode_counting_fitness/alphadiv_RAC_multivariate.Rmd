...
for (i in 1:nrow(tlike_data)) {
  ...
  for (j in seq_along(tlike_col_ranges)) {
    ...
    numeric_cols <- cols[sapply(tlike_data[, cols], is.numeric)]
    numeric_values <- as.numeric(tlike_data[i, numeric_cols])
    numeric_values <- numeric_values[!is.na(numeric_values)]
    if (length(numeric_values) > 0) {
      new_values[j] <- mean(numeric_values)
    }
  }
  tlike_data_meaned[i, names(tlike_col_ranges)] <- new_values
}
...
tlike_meaned_bactamb <- unlist(tlike_data_meaned[,4])
...
tlike_bactamb_ranked <- rankfit(tlike_meaned_bactamb)
...
tlike_bactamb_ranked <- tlike_bactamb_ranked[order(-tlike_bactamb_ranked$x),,drop=FALSE]
...
write.table(tlike_bactamb_ranked, file = "C:/Users/User/Desktop/bactamb_ranked.rnk", sep = "\t", quote = FALSE, row.names = TRUE)
