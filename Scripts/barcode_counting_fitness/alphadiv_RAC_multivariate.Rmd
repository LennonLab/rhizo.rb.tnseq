...
#remove intercept pval for fdr
for (obj_name in ls(anovafit_results)) {
  result <- anovafit_results[[obj_name]]
  anovafit_results[[obj_name]] <- result[-c(1, nrow(result)), ]
}

#extract all p-values from anovafit_results
all_pvalues <- unlist(lapply(anovafit_results, function(df) df[, 4]))

#perform FDR adjustment on all p-values
adjusted_pvalues <- p.adjust(all_pvalues, method = "fdr")

#split adjusted p-values back into separate vectors for each data frame
start_idx <- 1
for (obj_name in ls(anovafit_results)) {
  result <- anovafit_results[[obj_name]]
  end_idx <- start_idx + nrow(result) - 1
  result[, 4] <- adjusted_pvalues[start_idx:end_idx]
