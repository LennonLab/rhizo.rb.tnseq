...
#remove rows for sample id 25, 34, 36, and 38; reads were < 500,000
metadat = metadat %>% filter(Barseq_Index != "IT025" & Barseq_Index != "IT034" & Barseq_Index != "IT036" & Barseq_Index != "IT038")
...
diversity.fit <- diversity(goodcountdat[,1:5175],MARGIN = 1,index = "shannon")%>% 
  ...
  rename(shannon = ".")  %>%
  select("plant_ID","Nitrogen","Subpop","shannon")
...
kruskal.test(shannon ~ group, data = diversity.fit)
kruskal.test(NME ~ group, data = diversity.fit)
kruskal.test(richness ~ group, data = diversity.fit)

kruskal_pvals <- c(0.9827, 0.744, 0.08317)
...
potlabels <- c("1_nodN", "2_nodN", "3_nodN", "4_nodN", "6_nodamb", "7_nodamb", "8_nodamb", "9_nodamb", "0_nodamb", "1_bactN", "2_bactN", "3_bactN", "5_bactN", "7_bactamb", "9_bactamb", "0_bactamb", "t0", "t0", "t0", "t0", "t0") 
...
nodES_covar_permanova <- adonis2(covar_nodES ~ finalmetadat$Nitrogen_addition/finalmetadat$Subpop + finalmetadat$plantID, data = covar_nodES, permutations = 9999, method="euclidean")

nodNE_covar_permanova <- adonis2(covar_nodNE ~ finalmetadat$Nitrogen_addition/finalmetadat$Subpop + finalmetadat$plantID, data = covar_nodNE, permutations = 9999, method="euclidean")

complete_covar_permanova <- adonis2(covar_complete ~ finalmetadat$Nitrogen_addition/finalmetadat$Subpop + finalmetadat$plantID, data = covar_complete, permutations = 9999, method="euclidean")

Nmetab_covar_permanova <- adonis2(covar_Nmetab ~ finalmetadat$Nitrogen_addition/finalmetadat$Subpop + finalmetadat$plantID, data = covar_Nmetab, permutations = 9999,  method="euclidean")
...
RACmetadata <- metadat
RACmetadata <- RACmetadata[-c(17:21), ] #remove t0
RACmetadata <- RACmetadata %>% mutate(group = paste(Subpop, Nitrogen_addition, sep = "_")) #create a single-col group id
row.names(RACmetadata) <- c("021", "022", "023", "024", "026", "027", "028", "029", "030", "031", "032", "033", "035", "037", "039", "040")
row.names(goodcountdat) <- c("021", "022", "023", "024", "026", "027", "028", "029", "030", "031", "032", "033", "035", "037", "039", "040")
RACdat <- goodcountdat[, -c(5176:5178)]

#merged metadata/data df for re-subsetting and creating fitness rankings etc
RACsubsetter <- as.data.frame(cbind(RACdat, RACmetadata))
...
rankfitdata$V1[17:21] <- c("t0", "t0", "t0", "t0", "t0")
...
ggplot(covar_scores_nodES, aes(x = PC1, y = PC2, color = finalmetadat$Subpop, shape = finalmetadat$Nitrogen_addition)) +
  geom_point(size = 18) +
  xlim(c(-15,20)) +
  ylim(c(-15,15)) +
  xlab("PC1 (20%)") +
...
ggplot(covar_scores_nodNE,aes(x=PC1,y=PC2, color = finalmetadat$Subpop, shape = finalmetadat$Nitrogen_addition)) +
  geom_point(size=8)+
  xlab("PC 1 (18%)") +
  ylab("PC 2 (16%)") +
...
ggplot(covar_scores_complete,aes(x=PC1,y=PC2, color = finalmetadat$Subpop, shape = finalmetadat$Nitrogen_addition)) +
  geom_point(size=8)+
  xlab("PC 1 (18%)") +
  ylab("PC 2 (14%)") +
...
ggplot(covar_scores_Nmetab,aes(x=PC1,y=PC2, color = finalmetadat$Subpop, shape = finalmetadat$Nitrogen_addition)) +
  geom_point(size=8)+
  xlab("PC 1 (36%)") +
  ylab("PC 2 (25%)") +
...
for (df_name in df_names) {
  ...
  negatives <- rowSums(df[, 5:20] == -1)
  zeros <- rowSums(df[, 5:20] == 0)
  positives <- rowSums(df[, 5:20] == 1) #total up the frequency of 1s, 0s, and -1s in the fitness values of each df
...
for (df_name in ls(KEGGfit_env)) {
  ...
  df_positive <- data.frame(
    positive = c(positive_essential, positive_N) / 16,
    neutral = c(neutral_essential, neutral_N) / 16
  ) 
  ...
  df_negative <- data.frame(
    negative = c(negative_essential, negative_N) / 16,
    neutral = c(neutral_essential, neutral_N) / 16
  )
...
#comparison name generated as follows based on numbers retrieved  
  if (first_number %in% c("021", "022", "023", "024", "031", "032", "033", "035")) {
    first_name <- "+N"
  } else if (first_number %in% c("026", "027", "028", "029", "030", "037", "039", "040")) {
    first_name <- "-N"
  } else {
    first_name <- first_number
  }
  
  if (second_number %in% c("021", "022", "023", "024", "031", "032", "033", "035")) {
    second_name <- "+N"
  } else if (second_number %in% c("026", "027", "028", "029", "030", "037", "039", "040")) {
    second_name <- "-N"
  } else {
...
generate_subpopulation_name <- function(name) {
  ...
  if (first_number %in% c("021", "022", "023", "024", "026", "027", "028", "029", "030")) {
    first_name <- "Nodule"
  } else if (first_number %in% c("031", "032", "033", "035", "037", "039", "040")) {
    first_name <- "Bacteroid"
  } else {
    first_name <- first_number
  }
  
  if (second_number %in% c("021", "022", "023", "024", "026", "027", "028", "029", "030")) {
    second_name <- "Nodule"
  } else if (second_number %in% c("031", "032", "033", "035", "037", "039", "040")) {
...
ANOVAcomplete <- ANOVAcomplete[, -c(37:41)] 
ANOVAfitness <- ANOVAcomplete[, c(1, 21:36)]
anovastrata <- potlabels[-c(17:21)]
rownames(ANOVAfitness) <- ANOVAfitness$locus_tag
ANOVAfitness <- t(ANOVAfitness[, -c(1)])

#define treatment groups
ANOVAfitness <- data.frame(Ntreatment = c(rep("fertilized",4), rep("ambient",5),rep("fertilized", 4), rep("ambient", 3)), 
                            compartment = c(rep("nodule",9), rep("bacteroid",7)),
