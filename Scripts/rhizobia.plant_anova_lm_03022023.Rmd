...
ggplot(data=data %>% dplyr::filter(microbe != "pbs"), aes(x=nitrogen,y=dry.biomass)) +
     geom_boxplot()

ggplot(data=data %>% dplyr::filter(microbe != "pbs"), aes(x=date,y=dry.biomass)) +
...
biomass = lm(dry.biomass ~ nitrogen + date,data=data)
...
leveneTest(biomass_res ~ nitrogen * date, data = data) #not significant
#All looks okay
...
ggplot(data=data %>% dplyr::filter(microbe != "pbs"), aes(x=nitrogen,y=chlorophyll)) +
     geom_boxplot()

ggplot(data=data %>% dplyr::filter(microbe != "pbs"), aes(x=date,y=chlorophyll)) +
     geom_point() #if there looks to be a trend, add the time in the lm equation #filter functions remove PBs negative controls from dataset; can be further altered to look only at lib data rather than lib+WT

#Assess the effect of rhizobia presence over time points. Due to variation in magnitude of plant biomass, data was log10 transformed. I'd check without log10 first and see how the residuals look.

chlorophyll = lm(chlorophyll ~ nitrogen + date,data=data)
...
leveneTest(chlorophyll_res ~ nitrogen * date, data = data) #not significant
#All looks okay
...
ggplot(chlorophyll_plot, aes(x=nitrogen,y=mean)) +
  geom_point(size=5)+
  geom_errorbar(aes(ymin=mean-SE,ymax=mean+SE),width=0.3) +
  ylab(expression(atop(~Log[10]~"(chlorophyll content [mg])",paste("(mean \U00B1 SE, n = 5)")))) +
  scale_y_continuous(limits=c(0.85,4),breaks=seq(1,4,by=1))+
  theme_bw(base_size = 30) +
  theme(panel.grid = element_blank(),
        panel.border = element_rect(linewidth = 1, colour = "black",fill=NA),
...
nodbiomass = lm(nodule.biomass ~ nitrogen + date,data=librarymass)
...
height = lm(height ~ nitrogen + date,data=data)
...
ggplot(height_plot, aes(x=nitrogen,y=mean)) +
  geom_point(size=5)+
  geom_errorbar(aes(ymin=mean-SE,ymax=mean+SE),width=0.3) +
  ylab(expression(atop(~Log[10]~"(height [cm])",paste("(mean \U00B1 SE, n = 5)")))) +
  scale_y_continuous(limits=c(0.85,4),breaks=seq(1,4,by=1))+
  theme_bw(base_size = 30) +
  theme(panel.grid = element_blank(),
        panel.border = element_rect(linewidth = 1, colour = "black",fill=NA),
