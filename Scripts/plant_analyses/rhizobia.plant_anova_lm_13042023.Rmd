...
#t-test to assess potential association between nitrogen and dry plant biomass, factoring in apo plots by dividing by the average

ttest_mass <- t.test((libdata$dry.biomass/biomass_apos) ~ libdata$nitrogen, data=libdata, var.equal=TRUE)

ttest_mass

ttest_rhizmass <- t.test((dry.biomass/biomass_apos) ~ microbe, data=subset(data, microbe!="pbs"), var.equal=FALSE)
...
# biomass = lm(log10(dry.biomass) ~ nitrogen,data=subset(data,microbe=="lib"))
...
ttest_rhizchloro <- t.test((chlorophyll/chloro_apos) ~ microbe, data=subset(data, microbe!="pbs"), var.equal=FALSE)
...
ggplot(data=subset(plotmeans_data, microbe != "wt"), aes(x=nitrogen, y=meanchloro, color=microbe)) +
  ...
  geom_errorbar(aes(ymin=meanchloro-CIchloro, ymax=meanchloro+CIchloro), width=0.3) +
    ylab(expression(atop("Chlorophyll content [SPAD]",paste("(mean \U00B1 95% CI, n = 5)")))) +
...
# chlorophyll = lm(log10(chlorophyll) ~ nitrogen,data=subset(data,microbe=="lib"))
...
ttest_rhizleaves <- t.test((leaves/leaves_apos) ~ microbe, data=subset(data, microbe!="pbs"), var.equal=FALSE)
...
#Linear model for residuals
# leafcount = lm(leaves ~ nitrogen,data=subset(data, microbe=="lib"))
 leafcount = lm(log10(leaves) ~ nitrogen,data=subset(data, microbe=="lib"))
...
ggplot(data=data %>% dplyr::filter(microbe != "pbs"), aes(x=nitrogen,y=height,color=microbe)) +
     geom_boxplot()

ggplot(data=data %>% dplyr::filter(microbe != "pbs"), aes(x=date,y=height,color=microbe)) +
     geom_point() #if there looks to be a trend, add the time in the lm equation

#t-test to assess potential association between nitrogen and dry plant biomass, factoring in apo plots by dividing by the average

ttest_height <- t.test((libdata$height/height_apos) ~ libdata$nitrogen, data=libdata, var.equal=TRUE)

ttest_height

ttest_rhizheight <- t.test((height/height_apos) ~ microbe, data=subset(data, microbe!="pbs"), var.equal=FALSE)
...
height = lm(height ~ nitrogen,data=subset(data,microbe=="lib"))
 # height = lm(log10(height) ~ nitrogen,data=subset(data,microbe=="lib"))
...
``` {r}
#weighted anova to analyze nodule biomass

nodbiomass.anova <- aov(nodule.biomass ~ nitrogen, data=libdata, weight=nodule.count)

summary(nodbiomass.anova)
